                ...
                <form action="{{ route('update.profile.image') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <label for="profile-image-input" style="cursor:pointer;">
                        <img class="profile-image"
                            src="{{ $user->profile_image ? asset('' . $user->profile_image) : asset('assets/example/sayfood_profile.png') }}"
                            alt="{{ __('profile.profile_image_alt') }}" id="profile-image-preview">
                        <input type="file" name="profile_image" id="profile-image-input" accept="image/*"
                            style="display:none;" onchange="document.getElementById('profile-image-form').submit();">
                    </label>
                </form>
            </div>
            <div class="profile-details container-fluid flex-1 flex-column align-items-center justify-content-center">
                ...
                <form action=" {{ route('update.profile') }} " method="POST"
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" class="oswald form-control" id="username" name="username" autocomplete="on"
                    ...
                    @if (Auth::user()->role === 'customer')
                        <div class="form-group mb-3">
                            ...
                            <input type="date" class="oswald form-control" id="dob" name="dob"
                                autocomplete="on" value="{{ $user->customer->dob }}">
                            @error('dob')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="oswald form-control" id="address" name="address"
                                autocomplete="on" value="{{ $user->customer->address }}">
                            @error('address')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div
                        class="profile-update-btn-container d-flex flex-row justify-content-center align-items-center py-3">
                        <div class="row justify-content-around">
                            <a href="{{ route('profile') }}" class="col-sm-5 btn oswald profile-update-btn"
                                dusk="cancel-changes-btn">{{ __('profile.cancel_changes_button') }}</a>
                            <button type="submit" class="col-sm-5 btn oswald profile-update-btn"
                                dusk="save-changes-btn">{{ __('profile.save_changes_button') }}</button>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <div class="container-fluid d-flex justify-content-center align-items-center">
                @if (Auth::user()->role === 'customer')
                    <div class="profile-options justify-content-around row mt-5">
                        <form id="login-as-restaurant-form" method="POST" action="{{ route('login.as.restaurant') }}"
                        ...
                        <a href="{{ route('login.as.restaurant') }}"
                            ...
                            <img src="{{ asset('assets/profile_option_login_as_restaurant.png') }}" class="p-2"
                                alt="icon">
                        ...
                        <a href="{{ route('password.request') }}"
                            class="profile-option mb-2 col-md-5 d-flex flex-row align-items-center justify-content-start px-0">
                            <img src="{{ asset('assets/profile_option_reset_password.png') }}" class="p-2"
                                alt="icon">
