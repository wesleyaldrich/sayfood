...
@foreach ($orderStatuses as $index => $orderData)
        ...
        <div class="card-header activity-header d-flex align-items-center justify-content-between">
            ...
            <div class="d-flex align-items-center activity-item-actions gap-2">
                ...
                <button class="btn btn-sm px-3 dropdown-toggle" type="button"
                    ...
                    data-bs-toggle="collapse" data-bs-target="#order-card-{{ $orderData['orderId'] }}"
                    aria-expanded="false" aria-controls="order-card-{{ $orderData['orderId'] }}">
                    See Details
                </button>
            </div>
        </div>
            ...
            <div class="card-body details-section p-4">
                <div class="row">
                    ...
                    <div class="col-12 col-md-8 col-lg-6 mb-2 mb-md-0">
                        <div class="order-status-timeline-container d-flex justify-content-between align-items-center" style="overflow-y: hidden">
                            @php
                                $statuses = [
                    ...
                    @if(!empty($orderData['reviewButtonText']))
                        <div class="col-12 col-xl-2 align-items-end">
                            <button type="button" class="btn review-order-btn px-5 py-2 rounded-pill"
                                    data-bs-toggle="modal" data-bs-target="#reviewModal{{ $orderData['orderId'] }}">
    ...
    @if(!empty($orderData['reviewButtonText']) && $orderData['reviewButtonText'] === 'Review Order')
        <div class="modal fade" id="reviewModal{{ $orderData['orderId'] }}" tabindex="-1" aria-labelledby="reviewModalLabel{{ $orderData['orderId'] }}" aria-hidden="true">
