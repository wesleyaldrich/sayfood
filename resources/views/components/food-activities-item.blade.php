...
@foreach ($orderStatuses as $index => $orderData)
    ...
    <div class="card activity-main-item mb-2" id="card-detail">
        <div class="card-header activity-header d-flex align-items-center justify-content-between">
            ...
            @php
                $readyPickupKey = match ($orderData['readyPickupText']) {
                    'Waiting for Confirmation'     => 'activity.waiting_confirmation',
                    'Ready to Pick Up'   => 'activity.ready_to_pick_up',
            ...
            <div class="btn btn-sm px-3 btn-ready-pickup">{{ __($readyPickupKey) }}</div>
                ...
                <div class="row">
                    <div class="col-12 col-md-4">
                        <div class="order-items gap-2">
                            <p class="details-title">{{ __('activity.detailsTitle') }}</p>
                            @foreach ($orderData['items'] as $item)
                                <div class="row mb-1">
                                    ...
                                    @if (!empty($item['notes']))
                                        <div class="col-12 text-muted ps-2 small">{{ __('activity.note') }}: {{ $item['notes'] }}</div>
                                    @endif
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="col-12 col-md-8 col-lg-6 mb-2 mb-md-0">
                        <div class="order-status-timeline-container d-flex justify-content-between align-items-center"
                            ...
                            @php
                                $statuses = [
                                    __('activity.waiting_confirmation') => 'order_created',
                                    __('activity.ready_to_pick_up') => 'ready_to_pickup',
                                    __('activity.completed') => 'order_completed',
                                    __('activity.reviewed') => 'review_order',
                                ];
                    ...
                    @if(!empty($orderData['reviewButtonText']))
                        <div class="col-12 col-xl-2 align-items-end">
                            <button type="button" class="btn review-order-btn px-5 py-2 rounded-pill" data-bs-toggle="modal"
                                ...
                                {{ $orderData['reviewButtonText'] === 'Review Order' ? __('activity.review_order') : $orderData['reviewButtonText'] }}
                            </button>
                        </div>
                    @endif
                </div>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content custom-review-modal">
                    ...
                    <div class="modal-body text-center d-flex flex-column gap-5">
                        <h5 class="mb-3" style="font-family: 'Lato'">{{ __('activity.howWasYourFood') }}</h5>
